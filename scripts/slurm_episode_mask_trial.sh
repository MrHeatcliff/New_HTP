#!/usr/bin/env bash
#SBATCH --job-name=episode-mask-trial
...
#SBATCH --output=/home/vn-user0101/Dat/HTS-Dreamer/slurm_logs/episode-mask-%j.out
#SBATCH --error=/home/vn-user0101/Dat/HTS-Dreamer/slurm_logs/episode-mask-%j.err
set -euo pipefail
: "${SLURM_JOB_ID:?Submit using sbatch}"
readonly REPO=/home/vn-user0101/Dat/HTS-Dreamer
readonly OUTPUT="$REPO/paper_artifacts/persistence_research/episode_mask_${SLURM_JOB_ID}"
cd "$REPO"
mkdir -p "$OUTPUT"
...
exec "$REPO/.venv/bin/python" -m corewm_eval.coarse_target_trial "$OUTPUT" --factor episode_mask
