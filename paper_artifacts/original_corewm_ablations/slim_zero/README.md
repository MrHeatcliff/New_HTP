...
Bỏ projection và reconstruction; multi-stride prediction đọc prefix trực tiếp của h có chiều 2560. Các biến thể tách gradient tới backbone, prediction weight và variance penalty. Penalty variance không phải đầy đủ các thành phần VICReg.

Đối chứng trực tiếp: **`baseline`** ([thư mục](../baseline/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `baseline` | Ablation `slim_zero` |
|---|---|---|
| `agent.htp.enabled` | `false` | `true` |
| `agent.htp.grad_to_backbone` | `false` | `true` |
| `agent.htp.pdyn.dims` | `[128, 256, 512, 1024, 2048]` | `[128, 256, 512, 1024, 2560]` |
| `agent.htp.use_proj` | `true` | `false` |
| `agent.htp.use_recon` | `true` | `false` |
| `agent.loss_scales.htp_pdyn` | `1.0` | `0.0` |
...
```json
{
  ...
  "agent.htp.grad_to_backbone": true,
  "agent.htp.pdyn.dims": [
    128,
    256,
    512,
  ...
  "agent.htp.use_proj": false,
  "agent.htp.use_recon": false,
  "agent.htp.use_vicreg": false,
  "agent.loss_scales.htp_pdyn": 0.0,
