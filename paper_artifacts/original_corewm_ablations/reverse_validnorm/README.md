...
Chuẩn hóa prediction loss bằng số cặp hợp lệ thay vì trung bình cả B×T chứa zero-padding ở đuôi sequence. Phép đổi denominator được tách khỏi thay đổi stride.

Đối chứng trực tiếp: **`reverse`** ([thư mục](../reverse/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `reverse` | Ablation `reverse_validnorm` |
|---|---|---|
| `agent.htp.pdyn.normalize_valid` | `false` | `true` |
...
```json
{
  ...
  "agent.htp.pdyn.normalize_valid": true,
