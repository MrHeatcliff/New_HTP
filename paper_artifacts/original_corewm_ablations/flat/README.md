...
Nhóm factorial 2×2 của hai auxiliary loss. Giữ projection và các head; hệ số loss quyết định nhánh nào đóng góp gradient. Reconstruction khôi phục h từ các residual block; prediction dự đoán prefix tương lai với chuỗi action ở stride tương ứng.

Đối chứng trực tiếp: **`baseline`** ([thư mục](../baseline/)). Có trong Excel: `htp_empty_projection`.
...
| Trường | Đối chứng `baseline` | Ablation `flat` |
|---|---|---|
| `agent.htp.enabled` | `false` | `true` |
| `agent.loss_scales.htp_pdyn` | `1.0` | `0.0` |
| `agent.loss_scales.htp_rec` | `1.0` | `0.0` |
