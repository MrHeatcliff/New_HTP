# reset_mask_validnorm

## Ablation thực hiện gì?

Loại các cặp prediction có reset giữa nguồn và đích. normalize_valid quyết định chia cho số cặp hợp lệ hay giữ denominator B×T gốc.

Đối chứng trực tiếp: **`full_validnorm`** ([thư mục](../full_validnorm/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `full_validnorm` | Ablation `reset_mask_validnorm` |
|---|---|---|
| `agent.htp.pdyn.mask_resets` | `false` | `true` |
...
```json
{
  ...
  "agent.htp.pdyn.mask_resets": true,
