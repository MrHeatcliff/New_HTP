...
Thay linear output chung bằng output head riêng cho từng block, nhưng vẫn giữ shared trunk. Cấu hình không tạo các trunk độc lập và không bảo đảm độc lập semantics.
...
| Trường | Đối chứng `flat` | Ablation `separable_flat` |
|---|---|---|
| `agent.htp.proj.separable` | `false` | `true` |
