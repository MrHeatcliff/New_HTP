...
Cho loss của prefix sau truyền gradient vào phần context của các prefix trước, thay vì chỉ phần block mới. Forward input và target vẫn giữ nguyên.
...
| Trường | Đối chứng `full` | Ablation `pdyn_no_sg` |
|---|---|---|
| `agent.htp.pdyn.isolate_previous` | `true` | `false` |
...
```json
{
  ...
  "agent.htp.pdyn.isolate_previous": false,
