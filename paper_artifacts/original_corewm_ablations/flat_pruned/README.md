...
Bỏ hẳn các auxiliary head được chỉ định, thay vì giữ head rồi nhân loss bằng 0. Bởi vậy parameter count và lịch sử khởi tạo/RNG có thể khác đối chứng zero-loss.

Đối chứng trực tiếp: **`flat`** ([thư mục](../flat/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `flat` | Ablation `flat_pruned` |
|---|---|---|
| `agent.htp.use_pdyn` | `true` | `false` |
| `agent.htp.use_recon` | `true` | `false` |
| `agent.loss_scales.htp_pdyn` | `0.0` | `1.0` |
| `agent.loss_scales.htp_rec` | `0.0` | `1.0` |
