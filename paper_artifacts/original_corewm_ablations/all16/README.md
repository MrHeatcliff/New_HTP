...
Thay cách gán stride cho các cumulative prefix. Mỗi predictor nhận prefix hiện tại và chuỗi action can thiệp để dự đoán prefix tại endpoint; đây không phải successor prediction.

Đối chứng trực tiếp: **`full`** ([thư mục](../full/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `full` | Ablation `all16` |
|---|---|---|
| `agent.htp.pdyn.enforce_coarse_to_fine` | `true` | `false` |
| `agent.htp.pdyn.strides` | `[16, 8, 4, 2, 1]` | `[16, 16, 16, 16, 16]` |
...
```json
{
  ...
  "agent.htp.pdyn.enforce_coarse_to_fine": false,
  "agent.htp.pdyn.strides": [
    16,
    16,
    16,
    16,
