...
Đối chứng DreamerV3: tắt HTP, các readout dùng trạng thái RSSM h; không có hai auxiliary loss HTP.

Đối chứng trực tiếp: **`full`** ([thư mục](../full/)). Có trong Excel: `baseline`.
...
| Trường | Đối chứng `full` | Ablation `baseline` |
|---|---|---|
| `agent.htp.enabled` | `true` | `false` |

<details><summary>Overrides đăng ký cho cấu hình này</summary>

```json
{
  "agent.htp.enabled": false,
