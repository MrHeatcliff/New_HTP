...
Thay tensor action conditioning bằng zero, giữ nguyên kích thước tensor và parameter shapes của prediction heads.
...
| Trường | Đối chứng `full` | Ablation `no_actions` |
|---|---|---|
| `agent.htp.pdyn.zero_actions` | `false` | `true` |
...
```json
{
  ...
  "agent.htp.pdyn.zero_actions": true,
