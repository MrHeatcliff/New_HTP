...
Thay target projection EMA bằng online projection đã detach, hoặc thay tốc độ cập nhật EMA. Target prediction luôn stop-gradient; online_target không phải backprop vào target.
...
| Trường | Đối chứng `full` | Ablation `online_target` |
|---|---|---|
| `agent.htp.ablation_online_target` | `false` | `true` |

<details><summary>Overrides đăng ký cho cấu hình này</summary>

```json
{
  "agent.htp.ablation_online_target": true,
