...
Thay đường gradient qua tổng residual reconstruction hoặc chỉ giám sát full prefix. final_only được so với recon_no_sg: nếu giữ stop-gradient mà chỉ dùng final loss, các residual head trước sẽ không nhận supervision từ loss đó.

Đối chứng trực tiếp: **`recon_no_sg`** ([thư mục](../recon_no_sg/)). Đối chứng bổ sung ngoài các dòng gốc trong Excel.
...
| Trường | Đối chứng `recon_no_sg` | Ablation `recon_final_only` |
|---|---|---|
| `agent.htp.recon.final_only` | `false` | `true` |
...
```json
{
  ...
  "agent.htp.recon.final_only": true,
  "agent.htp.recon.stop_previous": false,
