...
class TaskHarmonizer(nj.Module):
  """Rectified HarmonyDream scalar: exp(-s) L + softplus(s)."""
  def __call__(self, loss):
    s = self.value('log_scale', jnp.zeros, (), jnp.float32)
    weight = jnp.exp(-s)
    return weight*loss+jax.nn.softplus(s), weight, s
